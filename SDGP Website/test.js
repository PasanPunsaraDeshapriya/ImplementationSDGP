      ...
      fs.createReadStream('datasets/creative-arts.csv')
        ...
        .on('end', () => {
          ...
          fs.readFile('test.html', 'utf8', function(err, html) {
            ...
            const finalHTML = html.replace('{{iT}}', selectiT).replace('{{education}}', selectEducation).replace('{{marketing}}', selectMarketing).replace('{{healthcare}}', selectHealthcare).replace('{{creative}}', selectCreative).replace("{{responce}}", selectCreative);
